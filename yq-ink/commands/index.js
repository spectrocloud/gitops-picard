...
function getStoreTagValue(store, key) {
  const tagString = (store?.cluster_tags || []).find((tag) => {
    const [tagKey] = tag.split(TAG_SEPARATOR);
    return key === tagKey;
  });
  ...
  return tagString.split(TAG_SEPARATOR)?.[1]
}
    ...
    if (activeField === 1) {
      ...
      const names = _.uniq(
        stores.reduce((accumulator, store) => {
          const tags = (store?.cluster_tags || []).map(
            (tag) => tag.split(TAG_SEPARATOR)[0]
          );
      ...
      const options = names.reduce((accumulator, key) => {
        accumulator[key] = _.uniq(
          stores.map((store) => {
            return getStoreTagValue(store, key)
          }).filter(Boolean)
        );
        return accumulator;
      }, {});
  ...
  const affectedStores = useMemo(() => {
    return allStores.filter((store) => {
      const conditions = data?.filters?.map((filter) => {
        ...
        return filter.value.includes(getStoreTagValue(store, filter.tag));
      });
      ...
      return filterOps.every((op, index, arr) => {
        ...
        if (op === "and") {
          return !!(
            conditions[index + firstMod] && conditions[index + secondMod]
          );
        }
        if (op === "or") {
          return !!(
    ...
    if (filterStep === "pending" && activeField === (filterCount + 1) * 2 + 1) {
      ...
      return (
        <>
          ...
          <Text>
            <Text bold>A</Text>(nd) <Text bold>O</Text>(r) <Text bold>C</Text>
            (onfirm)
          </Text>
        </>
      );
